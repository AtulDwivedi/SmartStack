...
<p class="pull-right"> 
<a class="btn btn-default btn-primary disabled" href="first.jsp" role="button">Questions</a> 
...
<li class="active"><a href="sort.spring?sortBy=top">Top problems</a></li>
...
<div class="col-md-10">
<p class="text-info"><b><a href="publicSolution.spring?quesId=${ques.quesId}">${ques.ques }</a></b></p>
...
<span class="pull-right"> by <a role="button">${ques.name }</a> on ${ques.date}</span> </div>
</div>
</c:forEach>
<!-- /Question-->
...
<!-- Paggination for Previous  -->
		 ...
		 <c:if test="${key.pageNo != 1}">
		   <li> <a href="indexReal.spring?page=${key.pageNo - 1}&pagesize=${key.pagesize}">Previous&nbsp;</a></li>
		</c:if>
	<!-- Paggination for Previous  -->
 ...
 <c:forEach begin="1" end="${(key.length)}" var="i">
				<c:choose>
					...
					<c:otherwise>
						 <li><a href="indexReal.spring?page=${i}&pagesize=${key.pagesize}">${i}</a></li>
					</c:otherwise>
				</c:choose>
</c:forEach>
...
<c:if test="${key.pageNo lt key.length}">
		<li><a href="indexReal.spring?page=${key.pageNo + 1}&pagesize=${key.pagesize}">Next&nbsp;</a></li>
</c:if>
<!-- Pagging for Next -->
